...
class SafarDriver(FetchDriver):
    ...
    def setup(self, opts):
        ...
        for i in range(0,len(data)):
            safar_city = self.add_timeseries('/'+data[i]['area']+'/pm2.5','AQI',description = 'PM2.5 readings for '+data[i]['area'],milliseconds=True)
            safar_city['Metadata'] = {}
    ...
    def process(self, data):
        ...
        for i in range(0,len(data)):
            a = self.add('/'+data[i]['area']+'/pm2.5', data[i]['pm2.5'])
            print(data[i]['pm2.5'])
